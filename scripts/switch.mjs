...
import { applyGatewayProfile, composeArgs, readVersions } from "./stack.mjs";
...
const env = { ...process.env, ...readVersions() };

function compose(profile, ...args) {
    return spawnSync("docker", ["compose", ...composeArgs(profile), ...args], { stdio: "inherit", env });
}
